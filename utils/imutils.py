...
def get_head_box_channel(x_min, y_min, x_max, y_max, width, height, resolution, coordconv=False):
    head_box = np.array([x_min/width*resolution[0], y_min/height*resolution[1], x_max/width*resolution[0], y_max/height*resolution[1]])
    head_box = head_box.astype(int)
    # head_box = np.clip(head_box, 0, resolution-1)
    if coordconv:
        unit_w = np.array(range(0,resolution[0]), dtype=np.float32)
        unit_h = np.array(range(0,resolution[1]), dtype=np.float32)
        head_channel = []
        for i in unit_w:
            head_channel.append([unit_h+i])
